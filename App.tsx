...
import { ViewType, Language, UserSettings, AccessControl, Student, School, StaffMember } from './types';
      ...
      <aside className={`bg-slate-900 text-white transition-all duration-500 ${isSidebarOpen ? 'w-64' : 'w-0 lg:w-20'} flex flex-col shrink-0 overflow-hidden relative z-50 shadow-2xl shadow-slate-900/50`}>
        ...
        <nav className="flex-1 py-6 px-3 space-y-2 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => (
            ...
            >
              <item.icon size={20} />
              {(isSidebarOpen || (window.innerWidth < 1024)) && <span className="font-bold text-sm">{item.label}</span>}
            </button>
          ))}
        </nav>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar">
          {showAuthScreen && !isAuthenticated ? (
            ...
            currentView === ViewType.SECURITY ? <SecuritySettings /> : <Dashboard />
          )}
        </div>
      </main>
    </div>
